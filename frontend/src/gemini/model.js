...
const API_KEY = "AIzaSyDtdrmGEg14TMExvW0yPZ5Z28CplTYCfgo";

const genAI = new GoogleGenerativeAI(API_KEY);
const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

export const extractJobInformation = async (prompt = "") => {
    // return;
    try {
        ...
        console.log("The Result", responce);
        return { success: true, response: responce };
    } catch (error) {
...
export const extractResumeInformation = async (path = "resume.pdf") => {
...
export const jobAnalysis = async (jobInfo, resumeInfo) => {
    try {
        const result = await model.generateContent({
            contents: [
                {
...
const main = async () => {
    ...
    if (job_info && resume) {
        const analysis = await jobAnalysis(job_info.response, resume.response);
        ...
        try {
            ...
            console.error("Error in job analysis:", e);
        }
    }
